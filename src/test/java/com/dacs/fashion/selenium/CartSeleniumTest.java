...
class CartSeleniumTest extends BaseSeleniumTest {
    ...
    void addProductToCart_thenCartPageShowsProduct() {
        ...
        assertTrue(
                driver.getPageSource().contains("Giỏ hàng"), "Phải mở được trang giỏ hàng"
        );
        assertFalse(
                driver.getPageSource().contains("Giỏ hàng đang trống"), "Sau khi thêm sản phẩm, giỏ hàng không được rỗng"
        );
        assertFalse(
                driver.findElements(By.xpath("//button[normalize-space()='Xóa']")).isEmpty(), "Giỏ hàng phải hiển thị nút Xóa sản phẩm"
        );
